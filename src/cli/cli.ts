...
import {
  ...
  discoverQueueDirs,
} from "../queue/directory_queue_source.ts";
import {
  BranchQueueOutput,
  createSinglePR,
...
if (effectiveCommand === "queue") {
  ...
  if (flags.file) {
    ...
    const queueYaml = resolve(queueDir, "queue.yaml");
    try {
      await Deno.stat(queueYaml);
    } catch {
      console.error(
        `Error: queue not found: .knox/queues/${flags.name}/queue.yaml`,
      );
      Deno.exit(2);
    }
    ...
    // ── Discovery mode ───────────────────────────────────────────────────────
    const queueDirs = await discoverQueueDirs(projectDir);
    if (queueDirs.length === 0) {
      console.error(
        `Error: no queues found in ${projectDir}/.knox/queues/`,
      );
      console.error(
        "Create a queue directory with a queue.yaml file, or use --file or --name.",
      );
      Deno.exit(2);
    }

    log.info(`Discovered ${queueDirs.length} queue(s).`);
